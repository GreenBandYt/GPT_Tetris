...
def clear_rows(grid):
    ...
    grid[:] = new_grid
    if cleared > 0:
        line_clear_sound.play()  # Проигрывание звука очистки строки
    return cleared
...
def show_menu():
    ...
    screen.blit(title_text, (size[0] // 2 - title_text.get_width() // 2, size[1] // 4))
...
def game_over_screen():
    ...
    screen.fill(black)
    font = pygame.font.SysFont(None, 55)
    over_text = font.render("Игра окончена", True, white)
    screen.blit(over_text, (size[0] // 2 - over_text.get_width() // 2, size[1] // 4))

    font = pygame.font.SysFont(None, 35)
    restart_text = font.render("Нажмите Enter чтобы начать заново", True, white)
    screen.blit(restart_text, (size[0] // 2 - restart_text.get_width() // 2, size[1] // 2))

    pygame.display.flip()

def game_loop():
    global shape, grid, done, paused, visible_image_height, line_height

    # Начальные параметры игры
    grid = [[0 for _ in range(size[0] // 20)] for _ in range(size[1] // 20)]
    shape = random.choice(shapes)()
    visible_image_height = 0
    speed = 3  # Начальная скорость падения фигур
    score = 0
    ...
    while not done:
        ...
        if not paused:
            if not shape.move_down(grid):
                ...
                score += filled_lines  # Увеличение счета
                speed += 0.1 * filled_lines  # Увеличение скорости при очистке строк

                # Обновление видимой части изображения
                if filled_lines > 0:
                    visible_image_height += filled_lines * line_height
                    if visible_image_height >= image_height:
                        # Остановка игры и отображение поздравления
                        win_sound.play()  # Проигрывание звука победы
                        grid = [[0 for _ in range(size[0] // 20)] for _ in range(size[1] // 20)]
                        font = pygame.font.SysFont(None, 35)
                        ...
                        pygame.display.flip()
                        pygame.time.wait(3000)  # Ожидание 3 секунд для показа поздравления
                        return False  # Выход из цикла игры

                shape = random.choice(shapes)()

                if not shape.move_down(grid):
                    game_over_screen()
                    pygame.time.wait(3000)  # Ожидание 3 секунд
                    return False  # Выход из цикла игры
